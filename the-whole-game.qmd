...
```{r}
bayes_control <- control_bayes(no_improve = 3L,
                               time_limit = 20,
                               save_pred  = TRUE,
                               verbose    = TRUE)
```
...
## Create Vetiver Model  {auto-animate="true"}
...
## Create Vetiver API {auto-animate="true"}
...
#| eval: false
pin_loc     <- 
  pins:::github_raw("Diverchris23/r-mlops/main/pins-r/_pins.yaml")
...
endpoint <- 
  vetiver_endpoint("https://lopescchristian-penguins.hf.space/predict")
...
original_metrics <-
  augment(endpoint, penguin_train_by_date) |> mutate(.pred_class = .pred_class %>% as_factor()) %>% 
